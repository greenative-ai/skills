...
curl -s -X POST -H 'content-type: application/json' \
  -d "$(printf '{"accesskey":"%s","secret":"%s","component":"entity","action":"..."}' "$GN_KEY" "$GN_SECRET" | base64)" \
  "$GN_ENDPOINT"
```
...
```bash
curl -s -X POST -H 'content-type: application/json' \
  -d "$(printf '{"accesskey":"%s","secret":"%s","component":"entity","action":"transfer","from":"storage1","to":"storage2","object":"data.csv"}' "$GN_KEY" "$GN_SECRET" | base64)" \
  "$GN_ENDPOINT"
```
...
```bash
curl -s -X POST -H 'content-type: application/json' \
  -d "$(printf '{"accesskey":"%s","secret":"%s","component":"entity","action":"transfer","from":"storage1","to":"storage2","filter":"2024-04"}' "$GN_KEY" "$GN_SECRET" | base64)" \
  "$GN_ENDPOINT"
```
...
```bash
curl -s -X POST -H 'content-type: application/json' \
  -d "$(printf '{"accesskey":"%s","secret":"%s","component":"entity","action":"transfer","from":"storage1","to":"db1","object":"data.csv","table":"my_table","worker":4}' "$GN_KEY" "$GN_SECRET" | base64)" \
  "$GN_ENDPOINT"
```
...
```bash
curl -s -X POST -H 'content-type: application/json' \
  -d "$(printf '{"accesskey":"%s","secret":"%s","component":"entity","action":"transfer","from":"db1","to":"storage1","table":"my_table"}' "$GN_KEY" "$GN_SECRET" | base64)" \
  "$GN_ENDPOINT"
```
