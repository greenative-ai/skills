...
curl -s -X POST -H 'content-type: application/json' \
  -d "$(printf '{"accesskey":"%s","secret":"%s","component":"kvs","action":"..."}' "$GN_KEY" "$GN_SECRET" | base64)" \
  "$GN_ENDPOINT"
```
...
```bash
curl -s -X POST -H 'content-type: application/json' \
  -d "$(printf '{"accesskey":"%s","secret":"%s","component":"kvs","action":"stores"}' "$GN_KEY" "$GN_SECRET" | base64)" \
  "$GN_ENDPOINT"
```
...
```bash
curl -s -X POST -H 'content-type: application/json' \
  -d "$(printf '{"accesskey":"%s","secret":"%s","component":"kvs","action":"create","key":"user1","value":{"name":"Alice","age":30}}' "$GN_KEY" "$GN_SECRET" | base64)" \
  "$GN_ENDPOINT"
```
...
```bash
curl -s -X POST -H 'content-type: application/json' \
  -d "$(printf '{"accesskey":"%s","secret":"%s","component":"kvs","action":"create","key":"user1","value":{"name":"Alice"},"store":"mystore"}' "$GN_KEY" "$GN_SECRET" | base64)" \
  "$GN_ENDPOINT"
```
...
```bash
curl -s -X POST -H 'content-type: application/json' \
  -d "$(printf '{"accesskey":"%s","secret":"%s","component":"kvs","action":"put","key":"user1","value":{"name":"Alice","age":31}}' "$GN_KEY" "$GN_SECRET" | base64)" \
  "$GN_ENDPOINT"
```
...
```bash
curl -s -X POST -H 'content-type: application/json' \
  -d "$(printf '{"accesskey":"%s","secret":"%s","component":"kvs","action":"get","key":"user1"}' "$GN_KEY" "$GN_SECRET" | base64)" \
  "$GN_ENDPOINT"
```
...
```bash
curl -s -X POST -H 'content-type: application/json' \
  -d "$(printf '{"accesskey":"%s","secret":"%s","component":"kvs","action":"delete","key":"user1"}' "$GN_KEY" "$GN_SECRET" | base64)" \
  "$GN_ENDPOINT"
```
...
```bash
curl -s -X POST -H 'content-type: application/json' \
  -d "$(printf '{"accesskey":"%s","secret":"%s","component":"kvs","action":"keys"}' "$GN_KEY" "$GN_SECRET" | base64)" \
  "$GN_ENDPOINT"
```
...
```bash
curl -s -X POST -H 'content-type: application/json' \
  -d "$(printf '{"accesskey":"%s","secret":"%s","component":"kvs","action":"keys","filter":"user","store":"mystore"}' "$GN_KEY" "$GN_SECRET" | base64)" \
  "$GN_ENDPOINT"
```
